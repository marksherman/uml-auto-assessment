...
# v1.3.2 2012/09/10 msherman added meaning to comments. Closed hanging opened files.
# v1.3.0 by jdefilippo up to 2012/09/05
...
def read_configuration_properties():
     ...
     # Test if the configuration file exists. It should be passed in as argv[1]
     if( len(sys.argv) != 2):
         #TODO cause error condition to throw
         print "Error encountered: incorrect number of arguments to script."
         return -1
         
    #TODO proper error handling here. "open" raises an IOError exception, which needs to be caught
    #Open the file here, with error checking, then use that file handle below.
     
     # A sectionless configuration file is passed generated by WebCAT, and named as the second argument to the plugin executable (argv[1]). ConfigParser can only read configuration files with section headers. To get aroudn this problem, the configuration is read in as a string with an arbitrary section header prepended to it.
...
# Pulls assignment data from a library of standard assignments, found in the script_Home folder.
# Currently considered a BAD thing, and is not called by execute.py
...
def grade_submission(user_Name, max_score_correctness):
     ...
     number_of_test_cases = configuration.number_of_test_cases #TODO: have it figure out number of tests as it runs, during the for loop
...
#This involves copying content into a file. This WILL PROBABLY NOT WORK on anything but
# the standard, static problem library #TODO also, is part of unit test
