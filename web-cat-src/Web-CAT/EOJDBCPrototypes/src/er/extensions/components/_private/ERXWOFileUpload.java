...
public class ERXWOFileUpload extends com.webobjects.appserver._private.WOFileUpload {
	...
	public void takeValuesFromRequest(WORequest worequest, WOContext wocontext) {
    	try {
            ...
            // SE: same code as above.  With Safari and some other browsers,
            // backtracking may lead to a submission that contains no data
            // for the file upload control, but this may result in a
            // RuntimeException exception instead of IllegalStateException
            // ("Could not create output stream. 'outputStream', or
            // 'streamToFilePath' may have evaluated to null. No Upload
            // performed.")
            boolean doThrow = !wocontext.hasSession() || !(wocontext.session() instanceof ERXSession) || !((ERXSession)wocontext.session()).didBacktrack();
            if(doThrow) {
                throw ex;
