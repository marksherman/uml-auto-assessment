...
checked_ptr<T>& checked_ptr<T>::operator++()
{
	...
	else if((char*)pointer > addr_info->user_end())
	{
		out_of_bounds = true;
...
checked_ptr<T>& checked_ptr<T>::operator--()
{
	...
	else if((char*)pointer < addr_info->user_begin())
	{
		out_of_bounds = true;
		__DMI->error(error_arithmetic_moved_out_of_bounds);
	}
...
checked_ptr<U> operator+(const checked_ptr<U>& ptr, ptrdiff_t delta)
{
	...
	else if((char*)new_pointer < addr_info->user_begin() ||
	   (char*)new_pointer > addr_info->user_end())
	{
		out_of_bounds = true;
		__DMI->error(error_arithmetic_moved_out_of_bounds);
	}
...
void checked_ptr<T>::store_type_info(mem_info* addr_info)
{
	...
	addr_info->element_size = sizeof(value_type);
	addr_info->cookie_size = cookie_calculator<value_type>::size();
