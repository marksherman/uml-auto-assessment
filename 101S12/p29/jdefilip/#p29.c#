...
int gameLogic(char a, char b)
{
  ...
  if  ( ( a == spock ) && ( ( b == scissors ) || ( b == rock ) ) )
    	        return player1;
  else if  ( ( a == scissors ) && ( ( b == paper ) || ( b == lizard ) ) )
                return player1; 
  else if ( ( a == paper ) && ( ( b == spock ) || ( b == rock ) ) ) 
                return player1;
  else if ( ( a == rock ) && ( ( b == scissors ) || ( b == lizard ) ) ) 
                return player1;
  else if ( ( a == lizard ) && ( ( b == paper ) || ( b == spock ) ) ) 
                return player1; 
  else if ( a == b )
		return draw;
  /* if a player1 or draw win does not happen, then player2 must win by definition (all rounds are determinate) */ 
  else 
                return player2; 
}
